...
import up.visulog.analyzer.CountMergeCommitsPerAuthorPlugin.Result;
import up.visulog.config.Configuration;
import up.visulog.gitrawdata.Commit;

public class CountCommitsWithDescriptionPerAuthorsPlugin implements AnalyzerPlugin {
	 private final Configuration configuration;
	    private Result result;
	    
	    public CountCommitsWithDescriptionPerAuthorsPlugin(Configuration config) {
	    ...
	    public static Result processLog(List<Commit> gitLog) { // methode static processLog qui prend comme argument une liste de type commit que l'on nome gitLog
	        ...
	        for (var commit : gitLog) {  // parcourrire gitLog 
	            if (commit.description.length()>0) {
