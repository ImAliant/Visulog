...
public class CountRemoveLinePerAutor implements AnalyzerPlugin{
	
	private final Configuration configuration; 
    private Result result;
    
    public CountRemoveLinePerAutor(Configuration generalConfiguration) {// constructeur de la classe (en public)
    ...
    static Result processLog(List<Commit> gitLog) { // methode static processLog qui prend comme argument une liste de type commit que l'on nome gitLog
        ...
        for (var commit : gitLog) {  
            ...
            result.commitsPerAuthor.put(commit.author, nb + commit.nbLineRemove); 
        }
